...
public class ConfigCheck {
    ...
    private void loadConfigFile() throws IOException {
        String configFilePath= absolutePath + File.separator + "configOfOriginal.properties";
        ...
        if (configFile.exists() && configFile.isFile() && configFile.length() > 0) {
            ...
            String backupConfigFilePath = absolutePath + File.separator + "back" + File.separator + "configOfOriginal.properties";
    ...
    private void checkConfigFileUpdates() throws IOException {
        ...
        if (lastModifiedTime > configLastModifiedTime) {
            log.info("configOfOriginal原始配置文件已更新，重新加载配置文件...");
            // 重新加载配置文件
            loadConfigFile();
            // 更新配置文件的最后修改时间
